...
/*   Updated: 2020/01/16 19:16:20 by nphilipp      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
t_wall_size	*calculate_wall(t_data *data, t_wall_size *wall_size)
{
	...
	wall_size->drawend = ((wall_size->lineheight / 2) + (data->map_data->dem_y / 2));
...
t_data	*raycasting(t_data *data)
{
	...
	while (x < data->map_data->dem_x)
	{
		...
		data->dda->ray_dir_x = data->map_data->dir_x + data->map_data->plane_x * camera_x;
		data->dda->ray_dir_y = data->map_data->dir_y + data->map_data->plane_y * camera_x;
		...
		//data->sprite->buffer[x] = data->dda->perpwall_dist;
		x++;
	}
	//data = sprites(data);
	return (data);
